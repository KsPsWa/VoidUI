-- ══════════════════════════════════════════════════════════════
--  VoidUI v4.0  ·  Complete Edition
--  Fixes: LetterSpacing removed, all layout bugs fixed
--  New: Theme system (6 presets + custom accent picker)
--       Tooltip system, Global hotkeys, Animations (fade-in)
--       CreateList, CreateGrid, CreateChart, CreateTree
--       Custom styled scrollbar, Tab icons improved
--       Settings tab (built-in), Resize handle
...
-- Активная тема (копия — не ссылка, чтобы можно было менять)
local T = {}
...
-- Fade-in анимация для нового элемента
local function fadeIn(obj, duration)
    obj.BackgroundTransparency = 1
    tw(obj, {BackgroundTransparency = 0}, TweenInfo.new(duration or 0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out))
end
...
--  ГЛОБАЛЬНЫЕ ХОТКЕИ
-- ══════════════════════════════════════════════════════════════
local _hotkeys = {}  -- { key=KeyCode, cb=func, label=string, enabled=bool }
...
local function attachTooltip(obj, text)
    ...
    obj.MouseEnter:Connect(function()
        ...
        local tw2 = #text * 6.5 + 20
        _tooltipFrame.Size = UDim2.new(0, tw2, 0, 24)
...
local function showContextMenu(items, x, y)
    ...
    mf2.ZIndex = 40; corner(mf2, 8); stroke(mf2, T.Border, 1, 0)
    ...
    for _, item in ipairs(items) do
        ...
        btn.ZIndex = 41; btn.Parent = mf2; corner(btn, 5)
...
function VoidUI:CreateStatsOverlay(opts)
    ...
    local sf = frm(_statGui, UDim2.new(0,W,0,H),
        UDim2.new(0, vp.X-W-14, 0, vp.Y-H-14), T.BgLight, 0)
    ...
    UserInputService.InputChanged:Connect(function(i)
        if drag2 and i.UserInputType==Enum.UserInputType.MouseMovement then
            ...
            sf.Position=UDim2.new(0,math.clamp(dOrigin.X.Offset+d.X,0,cvp.X-W),0,math.clamp(dOrigin.Y.Offset+d.Y,0,cvp.Y-H))
        end
    end)
...
function VoidUI:CreateWatermark(opts)
    ...
    _wmg=Instance.new("ScreenGui"); _wmg.Name="VoidUI_WM"
    _wmg.ResetOnSpawn=false; _wmg.IgnoreGuiInset=true; _wmg.Parent=LP.PlayerGui
...
local function _loadScreen(opts)
    ...
    local sg=Instance.new("ScreenGui"); sg.Name="VoidUI_Load"
    ...
    for i=1,3 do
        local d=frm(card,UDim2.new(0,6,0,6),UDim2.new(0,8+(i-1)*14,0,14),i==1 and T.Accent or T.AccentFaded,0)
        corner(d,99)
    end
    ...
    for i,step in ipairs(steps) do
        stp.Text=step
        tw(pb,{Size=UDim2.new(i/n,0,1,0)},TweenInfo.new(0.24,Enum.EasingStyle.Quad,Enum.EasingDirection.Out))
        task.wait(0.28)
    end
    task.wait(0.18)
...
function VoidUI:CreateWindow(opts)
    ...
    local W       = (opts.Size and opts.Size.Width)  or 660
    local H       = (opts.Size and opts.Size.Height) or 460
    ...
    local sg=Instance.new("ScreenGui"); sg.Name="VoidUI_Main"
    sg.ResetOnSpawn=false; sg.IgnoreGuiInset=true
    sg.ZIndexBehavior=Enum.ZIndexBehavior.Sibling; sg.Parent=LP.PlayerGui
    ...
    ca.Size=UDim2.new(1,-155,1,-42); ca.Position=UDim2.new(0,155,0,42)
    ...
    UserInputService.InputChanged:Connect(function(i)
        if drag2 and i.UserInputType==Enum.UserInputType.MouseMovement then
            ...
            mf.Position=UDim2.new(0,
                math.clamp(dOrigin2.X.Offset+d.X,0,cvp.X-W),0,
                math.clamp(dOrigin2.Y.Offset+d.Y,0,cvp.Y-(minimized and 42 or H)))
        end
    end)
    UserInputService.InputEnded:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 then drag2=false end
    ...
    minBtn.MouseButton1Click:Connect(function()
        ...
        tw(mf,{Size=UDim2.new(0,W,0,minimized and 42 or H)},TI.Mid)
    ...
    function Win:SetTheme(name)
        applyTheme(name)
        glw.ImageColor3=T.Accent; logoL.TextColor3=T.Accent
    end
    ...
    function Win:CreateTab(name, icon, badge)
        ...
        lst(scroll,5); pad(scroll,10,14,10,12)

        -- Hover эффект на скроллбар
        ...
        local function elem(h, hasDesc, noHover, noFade)
            local totalH = hasDesc and (h+20) or h
            local c=frm(scroll,UDim2.new(1,0,0,totalH),nil,T.BgElement,noFade and 0 or 1)
        ...
        function Tab:CreateSection(text)
            ...
            -- LetterSpacing убран (не существует в Roblox)
        end
        ...
        function Tab:CreateAlert(opts2)
            ...
            frm(c,UDim2.new(0,3,1,0),nil,ac2,0); corner(frm(c,UDim2.new(0,3,1,0),nil,ac2,0),3)
        ...
        function Tab:CreateToggle(opts2)
            ...
            ctxBtn.Size=UDim2.new(1,0,1,0); ctxBtn.BackgroundTransparency=1; ctxBtn.Text=""; ctxBtn.Parent=c
            ...
            ctxBtn.MouseButton1Click:Connect(function() set(not val); local x,y=mrel(c); ripple(c,x,y) end)
            ctxBtn.MouseButton2Click:Connect(function()
                local mx,my=mrel(sg:FindFirstChildOfClass("Frame") or c)
                local mp=UserInputService:GetMouseLocation()
                showContextMenu({
        ...
        function Tab:CreateDropdown(opts2)
            ...
            mb.MouseButton1Click:Connect(function()
                ...
                dd.Visible=true; tw(dd,{Size=UDim2.new(1,0,0,th2)},TI.Fast)
        ...
        function Tab:CreateStepper(opts2)
            ...
            local minusB=Instance.new("TextButton"); minusB.Size=UDim2.new(0,26,1,0); minusB.BackgroundTransparency=1
            minusB.Text="−"; minusB.Font=Enum.Font.GothamBold; minusB.TextSize=14; minusB.TextColor3=T.TextSub; minusB.BorderSizePixel=0; minusB.Parent=ctrlF
            ...
            local plusB=Instance.new("TextButton"); plusB.Size=UDim2.new(0,26,1,0); plusB.Position=UDim2.new(1,-26,0,0)
            plusB.BackgroundTransparency=1; plusB.Text="+"; plusB.Font=Enum.Font.GothamBold; plusB.TextSize=14; plusB.TextColor3=T.TextSub; plusB.BorderSizePixel=0; plusB.Parent=ctrlF
        ...
        function Tab:CreateAccordion(opts2)
            local aTitle=opts2.Title or "Accordion"; local aOpen=opts2.DefaultOpen or false
            ...
            inner.ClipsDescendants=true; corner(inner,5); lst(inner,4); pad(inner,4,0,4,0)

            local expanded=aOpen; local innerH=0
            local function recalcH()
                local total=0
                for _,ch in ipairs(inner:GetChildren()) do
                    if ch:IsA("Frame") then total+=ch.AbsoluteSize.Y+4 end
                end
                return total
            end
            ...
            local hBtn=Instance.new("TextButton"); hBtn.Size=UDim2.new(1,0,0,36); hBtn.BackgroundTransparency=1; hBtn.Text=""; hBtn.Parent=header
            hBtn.MouseButton1Click:Connect(toggleAcc)
            ...
            local function ielem(h2)
                ...
                cf.LayoutOrder=ilo(); corner(cf,5); stroke(cf,T.Border,1,0.5); return cf
            end
            function InnerTab:CreateToggle(o2)
                ...
                local tbg3=frm(cf,UDim2.new(0,34,0,18),UDim2.new(1,-42,0.5,-9),val2 and T.Accent or T.AccentFaded,0); corner(tbg3,9)
                local th3=frm(tbg3,UDim2.new(0,14,0,14),val2 and UDim2.new(1,-16,0.5,-7) or UDim2.new(0,2,0.5,-7),T.TextPrimary,0); corner(th3,7)
                ...
                rb2.MouseButton1Click:Connect(function()
                    ...
                    tCb2(val2)
                    task.defer(function()
                        innerH=recalcH()+12; wrapper.Size=UDim2.new(1,0,0,40+innerH); inner.Size=UDim2.new(1,-24,0,innerH)
                    end)
                end)
            end
            function InnerTab:CreateButton(o2)
                local bCb2=o2.Callback or function() end; local cf=ielem(28)
        ...
        function Tab:CreateBadge(opts2)
            local bCol=opts2.Color or T.Accent; local c=elem(32); reg(c,opts2.Name or "")
        ...
        function Tab:CreateList(opts2)
            local items=opts2.Items or {}
            local multi=opts2.Multi or false
            local lCb=opts2.Callback or function() end
            local selected=multi and {} or nil
            local itemH=opts2.ItemHeight or 30
            local maxVisible=opts2.MaxVisible or 5
            local visH=math.min(#items,maxVisible)*itemH+40

            local c=frm(scroll,UDim2.new(1,0,0,visH),nil,T.BgElement,1)
            c.LayoutOrder=lo(); corner(c,7); stroke(c,T.Border,1,0.4); reg(c,opts2.Name or "List")
            task.defer(function() tw(c,{BackgroundTransparency=0},TI.Mid) end)

            local nl2=lbl(c,opts2.Name or "List",12,T.TextPrimary,Enum.Font.GothamBold)
            nl2.Size=UDim2.new(1,-24,0,18); nl2.Position=UDim2.new(0,12,0,8)

            local listFrame=Instance.new("ScrollingFrame")
            listFrame.Size=UDim2.new(1,-24,0,visH-40)
            listFrame.Position=UDim2.new(0,12,0,32)
            listFrame.BackgroundTransparency=1
            listFrame.BorderSizePixel=0
            listFrame.ScrollBarThickness=3
            listFrame.ScrollBarImageColor3=T.ScrollBar
            listFrame.CanvasSize=UDim2.new(0,0,0,#items*itemH)
            listFrame.Parent=c
            lst(listFrame,0)

            local itemBtns={}

            local function selectItem(item,btn,lbl2)
                if multi then
                    selected[item]=not selected[item]
                    tw(btn,{BackgroundColor3=selected[item] and T.AccentFaded or T.Bg},TI.Fast)
                    tw(lbl2,{TextColor3=selected[item] and T.TextAccent or T.TextSub},TI.Fast)
                    lCb(selected)
                else
                    selected=item
                    for itm,parts in pairs(itemBtns) do
                        local active=itm==item
                        tw(parts.btn,{BackgroundColor3=active and T.AccentFaded or T.Bg},TI.Fast)
                        tw(parts.lbl,{TextColor3=active and T.TextAccent or T.TextSub},TI.Fast)
                    end
                    lCb(item)
                end
            end
            ...
            for _,item in ipairs(items) do
                local row=frm(listFrame,UDim2.new(1,0,0,itemH),nil,T.Bg,0)
                corner(row,4)

                -- Иконка отметки
                local checkL=lbl(row,multi and "□" or "○",11,T.TextMuted,Enum.Font.GothamBold,Enum.TextXAlignment.Center)
                checkL.Size=UDim2.new(0,18,1,0); checkL.Position=UDim2.new(0,6,0,0)

                local itemL=lbl(row,tostring(item),11,T.TextSub,Enum.Font.Gotham)
                itemL.Size=UDim2.new(1,-34,1,0); itemL.Position=UDim2.new(0,28,0,0)

                local rb=Instance.new("TextButton"); rb.Size=UDim2.new(1,0,1,0); rb.BackgroundTransparency=1; rb.Text=""; rb.Parent=row
                rb.MouseEnter:Connect(function() tw(row,{BackgroundColor3=T.BgHover},TI.Fast) end)
                rb.MouseLeave:Connect(function()
                    local active=multi and selected[item] or selected==item
                    tw(row,{BackgroundColor3=active and T.AccentFaded or T.Bg},TI.Fast)
                end)
                rb.MouseButton1Click:Connect(function()
                    selectItem(item,row,itemL)
                    checkL.Text=multi and (selected[item] and "■" or "□") or (selected==item and "●" or "○")
                    checkL.TextColor3=((multi and selected[item]) or selected==item) and T.Accent or T.TextMuted
                    local x,y=mrel(row); ripple(row,x,y)
                end)

                itemBtns[item]={btn=row,lbl=itemL,chk=checkL}
            end
            ...
            function obj:Set(v)
                selected=v
                for itm,parts in pairs(itemBtns) do
                    local active=multi and selected[itm] or selected==itm
                    parts.btn.BackgroundColor3=active and T.AccentFaded or T.Bg
                    parts.lbl.TextColor3=active and T.TextAccent or T.TextSub
                    parts.chk.Text=multi and (selected[itm] and "■" or "□") or (selected==itm and "●" or "○")
                end
            end
            function obj:SetItems(newItems)
                ...
                itemBtns={}; selected=multi and {} or nil
                listFrame.CanvasSize=UDim2.new(0,0,0,#newItems*itemH)
                for _,item in ipairs(newItems) do
                    local row=frm(listFrame,UDim2.new(1,0,0,itemH),nil,T.Bg,0); corner(row,4)
                    local itemL2=lbl(row,tostring(item),11,T.TextSub,Enum.Font.Gotham)
                    itemL2.Size=UDim2.new(1,-12,1,0); itemL2.Position=UDim2.new(0,10,0,0)
                    local rb2=Instance.new("TextButton"); rb2.Size=UDim2.new(1,0,1,0); rb2.BackgroundTransparency=1; rb2.Text=""; rb2.Parent=row
                    rb2.MouseButton1Click:Connect(function() selectItem(item,row,itemL2) end)
                    itemBtns[item]={btn=row,lbl=itemL2}
                end
            end
            return obj
        end

        -- ── Grid (сетка кнопок/переключателей) ───────────────
        function Tab:CreateGrid(opts2)
            local items=opts2.Items or {}
            local cols=opts2.Columns or 3
            local gCb=opts2.Callback or function() end
            local selected=opts2.Default
            local multi=opts2.Multi or false
            local selState={}

            local rows=math.ceil(#items/cols)
            local cellH=opts2.CellHeight or 36
            local totalH=32+rows*(cellH+4)+4

            local c=frm(scroll,UDim2.new(1,0,0,totalH),nil,T.BgElement,1)
            c.LayoutOrder=lo(); corner(c,7); stroke(c,T.Border,1,0.4); reg(c,opts2.Name or "Grid")
            task.defer(function() tw(c,{BackgroundTransparency=0},TI.Mid) end)

            local nl2=lbl(c,opts2.Name or "Grid",12,T.TextPrimary,Enum.Font.GothamBold)
            nl2.Size=UDim2.new(1,-24,0,18); nl2.Position=UDim2.new(0,12,0,8)

            local gridF=Instance.new("Frame")
            gridF.Size=UDim2.new(1,-24,0,rows*(cellH+4))
            gridF.Position=UDim2.new(0,12,0,32)
            gridF.BackgroundTransparency=1; gridF.Parent=c
            ...
            for i,item in ipairs(items) do
                ...
                local cellW=(1/cols)
                local xPos=col*cellW
                local yPos=row*(cellH+4)

                local cellF=frm(gridF,UDim2.new(cellW,-4,0,cellH),UDim2.new(xPos,2,0,yPos),
                    item==selected and T.AccentFaded or T.Bg,0)
                corner(cellF,6); stroke(cellF,T.Border,1,item==selected and 0 or 0.5)

                local cellIcon=lbl(cellF,item.icon or "",16,T.TextMuted,Enum.Font.GothamMedium,Enum.TextXAlignment.Center)
                cellIcon.Size=UDim2.new(1,0,0,item.icon and 20 or 0)
                cellIcon.Position=UDim2.new(0,0,0,item.icon and 4 or 0)

                local cellTxt=lbl(cellF,item.label or tostring(item),10,T.TextSub,Enum.Font.GothamMedium,Enum.TextXAlignment.Center)
                cellTxt.Size=UDim2.new(1,-4,0,14)
                cellTxt.Position=UDim2.new(0,2,1,-16)

                local rb=Instance.new("TextButton"); rb.Size=UDim2.new(1,0,1,0); rb.BackgroundTransparency=1; rb.Text=""; rb.Parent=cellF
                rb.MouseEnter:Connect(function()
                    local active=multi and selState[item] or selected==item
                    if not active then tw(cellF,{BackgroundColor3=T.BgHover},TI.Fast) end
                    tw(cellTxt,{TextColor3=T.TextPrimary},TI.Fast)
                end)
                rb.MouseLeave:Connect(function()
                    local active=multi and selState[item] or selected==item
                    tw(cellF,{BackgroundColor3=active and T.AccentFaded or T.Bg},TI.Fast)
                    tw(cellTxt,{TextColor3=active and T.TextAccent or T.TextSub},TI.Fast)
                end)
                rb.MouseButton1Click:Connect(function()
                    if multi then
                        selState[item]=not selState[item]
                        tw(cellF,{BackgroundColor3=selState[item] and T.AccentFaded or T.Bg},TI.Fast)
                        stroke(cellF,T.Border,1,selState[item] and 0 or 0.5)
                        tw(cellTxt,{TextColor3=selState[item] and T.TextAccent or T.TextSub},TI.Fast)
                        gCb(selState)
                    else
                        selected=item
                        for itm,parts in pairs(cellBtns) do
                            local active=itm==item
                            tw(parts.f,{BackgroundColor3=active and T.AccentFaded or T.Bg},TI.Fast)
                            tw(parts.t,{TextColor3=active and T.TextAccent or T.TextSub},TI.Fast)
                        end
                        gCb(item)
                    end
                    local x,y=mrel(cellF); ripple(cellF,x,y)
                end)

                cellBtns[item]={f=cellF,t=cellTxt,i=cellIcon}
            end

            local obj={}
            function obj:Get() return multi and selState or selected end
            function obj:Set(v)
                selected=v
                for itm,parts in pairs(cellBtns) do
                    local active=itm==v
                    parts.f.BackgroundColor3=active and T.AccentFaded or T.Bg
                    parts.t.TextColor3=active and T.TextAccent or T.TextSub
                end
            end
            return obj
        end

        -- ── Chart (мини-график значений) ─────────────────────
        function Tab:CreateChart(opts2)
            local chartH=opts2.Height or 80
            local maxPoints=opts2.MaxPoints or 60
            local label=opts2.Name or "Chart"
            local minVal=opts2.Min or 0
            local maxVal=opts2.Max or 100
            local color=opts2.Color or T.Accent
            local points={}

            local totalH=chartH+32
            local c=frm(scroll,UDim2.new(1,0,0,totalH),nil,T.BgElement,1)
            c.LayoutOrder=lo(); corner(c,7); stroke(c,T.Border,1,0.4); reg(c,label)
            task.defer(function() tw(c,{BackgroundTransparency=0},TI.Mid) end)

            local nl2=lbl(c,label,12,T.TextPrimary,Enum.Font.GothamBold)
            nl2.Size=UDim2.new(1,-80,0,18); nl2.Position=UDim2.new(0,12,0,8)
            local curVal=lbl(c,"0",11,T.TextAccent,Enum.Font.GothamBold,Enum.TextXAlignment.Right)
            curVal.Size=UDim2.new(0,65,0,18); curVal.Position=UDim2.new(1,-77,0,8)

            -- Сетка
            local chartArea=frm(c,UDim2.new(1,-24,0,chartH),UDim2.new(0,12,0,28),T.BgInput,0); corner(chartArea,5)
            stroke(chartArea,T.Border,1,0.2)

            -- Горизонтальные линии сетки
            for i=1,3 do
                local gridLine=frm(chartArea,UDim2.new(1,0,0,1),UDim2.new(0,0,i/4,0),T.Sep,0)
                local gridLbl=lbl(chartArea,tostring(math.round(minVal+(maxVal-minVal)*(1-i/4))),8,T.TextMuted,Enum.Font.Gotham)
                gridLbl.Size=UDim2.new(0,30,0,10); gridLbl.Position=UDim2.new(0,2,i/4,-5)
            end

            -- Бары/линии
            local bars={}
            local barW=1/maxPoints

            local function rebuildChart()
                for _,b in ipairs(bars) do if b.Parent then b:Destroy() end end
                bars={}
                local n=#points
                ...
                for i,v in ipairs(points) do
                    local norm=math.clamp((v-minVal)/(maxVal-minVal),0,1)
                    local x=(i-1)/maxPoints
                    -- Бар
                    local bar=frm(chartArea,UDim2.new(barW-0.002,0,norm,0),UDim2.new(x,0,1,-norm*chartH,0),color,0.5)
                    bar.AnchorPoint=Vector2.new(0,1)
                    bar.Position=UDim2.new(x,0,1,0)
                    bar.Size=UDim2.new(barW-0.002,0,norm,0)
                    table.insert(bars,bar)
                end
                -- Линия поверх
                if n>1 then
                    for i=2,n do
                        local v1=math.clamp((points[i-1]-minVal)/(maxVal-minVal),0,1)
                        local v2=math.clamp((points[i]-minVal)/(maxVal-minVal),0,1)
                        local x1=(i-2)/maxPoints; local x2=(i-1)/maxPoints
                        local dot=frm(chartArea,UDim2.new(0,3,0,3),UDim2.new(x2,0,1-v2,-1),color,0)
                        dot.AnchorPoint=Vector2.new(0.5,0.5)
                        corner(dot,99); dot.ZIndex=3
                        table.insert(bars,dot)
                    end
                end
            end
            ...
            function obj:AddPoint(v)
                ...
                curVal.Text=tostring(math.round(v*10)/10)
                rebuildChart()
            end
            function obj:Clear() points={}; rebuildChart(); curVal.Text="0" end
            function obj:SetRange(mn,mx) minVal=mn; maxVal=mx end
            function obj:SetColor(col) color=col end
            return obj
        end
        ...
        function Tab:CreateTable(opts2)
            ...
            local head=frm(c,UDim2.new(1,0,0,28),nil,T.AccentFaded,0); corner(head,7)
            lst(head,0,Enum.FillDirection.Horizontal)
            ...
            for ri,row in ipairs(rows) do
                local rf=frm(c,UDim2.new(1,0,0,rowH),UDim2.new(0,0,0,28+(ri-1)*rowH),ri%2==0 and T.BgLight or T.BgElement,0)
            ...
            function obj:AddRow(row)
                ...
                local rf=frm(c,UDim2.new(1,0,0,rowH),UDim2.new(0,0,0,28+ri*rowH),ri%2==0 and T.BgLight or T.BgElement,0)
                lst(rf,0,Enum.FillDirection.Horizontal)
                for _,cell in ipairs(row) do
                    local cl4=lbl(rf,tostring(cell),10,T.TextSub,Enum.Font.Gotham,Enum.TextXAlignment.Center)
                    cl4.Size=UDim2.new(colW,0,1,0)
        ...
        function Tab:CreateChips(opts2)
            ...
            local def2=opts2.Default; local multi=opts2.Multi or false
            local cCb=opts2.Callback or function() end
            local selected2=multi and {} or def2

            local c=frm(scroll,UDim2.new(1,0,0,56),nil,T.BgElement,1)
            c.LayoutOrder=lo(); corner(c,7); stroke(c,T.Border,1,0.4); reg(c,opts2.Name)
            task.defer(function() tw(c,{BackgroundTransparency=0},TI.Mid) end)
            ...
            for _,chip in ipairs(chips) do
                ...
                cf.MouseButton1Click:Connect(function()
                    if multi then
                        ...
                        cf.TextColor3=selected2[chip] and T.TextAccent or T.TextMuted
                        cCb(selected2)
                    else
                        selected2=chip
    ...
    function Win:CreateSettingsTab()
        ...
        sTab:CreateToggle({
            ...
            Description  = "Fade-in при создании",
            CurrentValue = true,
        ...
        sTab:CreateTextDisplay({Name="Автор",      Text="Enhanced"})
        sTab:CreateTextDisplay({Name="Тема",       Text="Void"})
